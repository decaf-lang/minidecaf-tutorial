...
在每阶段截止提交以后，我们一般会在一月内在网络学堂上公布成绩。
如果你认为成绩有问题，请及时与助教联系（关于实验成绩的疑问最晚请在期末考试以前提出，考试以后不能再更改实验成绩）。

## 补交政策

* 假设 a 日 24:00 是某个 stage 在网络学堂上的截止时间；
* a + k (k <= 14) 日 24:00 前补交，此 stage 得分乘以 1 - k/15；
* 提交/补交时间是 a + 14 日 24:00 前这个 stage 的 branch 最后一次触发 CI 的时间；
    * 准确来讲是创建 pipeline 的时间，而不是 pipeline 更新的时间，这几乎等于你 push 到远端仓库的时间。并且，你也可以随意地 retry 反复运行 CI，这不会对你的提交/补交产生影响。如果在多次 retry 中你的代码会得到不一致的结果，请联系助教。
* a + 14 日 24:00 后不接受补交，此 stage 得 0 分。
