...
> `int a = a + 1;`，这样一条声明完全等价于 `int a; a = a + 1;`，依据5.3，这是未定义行为。
>
> 引入块语句之后，可能会出现这样的程序片段：
> 
> ```c
> int a = 1;
> {
...
> }
> ```
> 这里可以存在另一种理解：块语句中的`a + 1`中的`a`可以理解成使用块外面`int a = 1;`定义的`a`，但是我们仍然规定`int a = a + 1;`等价于`int a; a = a + 1;`，所以`a + 1`中的`a`还是块里面未初始化的这个，还是认为这个程序存在未定义行为。
