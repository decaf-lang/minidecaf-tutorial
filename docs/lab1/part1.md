...
3. 学会开发中使用的工具和设计模式，包括 **ANTLR 工具**， **lalr1 工具**，**Visitor 模式**等。根据你选择的语言和实现方式，需要了解的内容可能是不一样的，不是所有内容都需要了解
...
>
> 有些实现中把所有关键字都作为一*种*token，认为关键字int和关键字return之间的关系就和上面的Integer(0) 和 Integer(222)之间的关系一样；另一些实现中为每种关键字设置一种token，认为关键字int和关键字return不是同一种token
...
> 这份代码主要目的只是让大家熟悉词法分析的概念和其中涉及到的问题，并不是作为一个实现的参考，后面的章节会给出更多实现的指导。如果你已经比较熟悉这个概念，只是不知道怎么实现，那它对你应该没有太大的帮助，理论上是可以跳过不看的（不过思考题中涉及到了它，所以还是得看）。
>
...
> 与minilexer类似，这份代码并不是作为实现上的指导，如果你已经比较熟悉语法分析的概念，只是不知道怎么实现，那它对你应该没有太大的帮助，理论上是可以跳过不看的（不过思考题中涉及到了它，所以还是得看）。
...
>
> 如果你选择的语言支持抽象数据类型和模式匹配，那么基本整个实验中都用不上Visitor 模式（当然也不是说不能用，只是说即使不用，表达程序逻辑也不会很麻烦，甚至可能更清晰），你可以跳过相关的教程。在这样的语言中遍历 AST是一件非常简单的事情，没有必要额外介绍方法。
