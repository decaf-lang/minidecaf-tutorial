...
# step10 语义规范
**10.1.** 全局变量可以被初始化，但初始值（initializer）必须是整数字面量。
> 其实初始值的值是常量即可，例如可以 `int a=2+3;`，编译器会计算 `2+3==5`。
> 不过为了简单，我们不要求支持 `int a=2+3;`，只要求 `int a=5;` 即可。
>
> 不过测例里不会出现上面那种，全局变量被一个不是字面量的常量初始化的情况。
> 所以如果有兴趣，你也可以支持 `int a=2+3;`。

**10.2.** 如果没有显式给出初始值，全局变量初始值为 0。

**10.3.** 不允许重复声明全局变量。
> 其实 C 是允许全局变量重复声明，只要不重复定义的。
> `int a; int a=2; int a; int main(){return a;}` 是合法的 C。
> 为了简单，我们不支持这点。
