...
我们推荐大家按照以下步骤实现，当然这不是唯一的实现方式。前中端的部分在前面的step中涉及很多，大家应该已经比较熟悉，这里着重关注后端要做的事。此处我们采用寄存器进行传参。

### 后端**主要**需要修改的文件
`backend/reg/bruteregalloc.py`和`backemd/riscv/riscvasmemitter.py`

### 要做什么
1. 调用者需要按照调用约定放置参数，这会涉及寄存器保存、参数移动到正确的位置等操作，因此需要修改寄存器分配过程。对应`backend/reg/bruteregalloc.py`。
2. 被调用者需要从正确的位置拿到参数，以及处理上下文信息（包括返回地址、callee-save寄存器等），这些信息大多都是在函数开始时和结束时保存和恢复，因此需要修改函数汇编生成部分代码。对应`backemd/riscv/riscvasmemitter.py`。

### 对于调用者的处理

这里我们需要关注`BruteRegAlloc`类，我们首先需要处理调用者的参数放置，为了按照调用约定放置参数，我们应该将原来物理寄存器中的临时变量都保存下来，才能将参数移动到相应的寄存器。

1. **保存活跃变量：**`localAlloc`函数处，我们可以使用`isinstance`来判断一条指令是不是一条`Call`指令（你也可以自己保存别的信息在`TACInstr`中用来辅助判断），如果是则保存所有被占用的caller-save寄存器，并且将他们与临时变量解绑。`subEmitter.emitStoreToStack`可以帮助我们用来把一个变量放在栈上，`unbind`函数可以用来解绑寄存器的绑定关系（通过`Riscv`类可以找到所有物理寄存器）。

2. **放置参数：**特殊处理`allocForLoc`函数，`allocForLoc`会为每一条Tac指令分配寄存器，但是我们对于函数应该特殊处理，因为对于其他指令来说，分配的寄存器可以是随机的，但是函数调用要使用固定的寄存器。所有传参用到的寄存器(a0~a7)都是caller-save寄存器，1中的操作保证了传参所需要的寄存器都是空的，因此直接将参数放到寄存器中即可。具体地，用物理寄存器`a0`~`a7`传递被调用函数的前8个参数，我们假设这8个参数对应的临时变量（Temp）为`v0`~`v7`。对于第i个参数，目标是将`vi`的值加载入`ai`。若`vi`已经与某个物理寄存器`xj`绑定，则可以生成指令`mv ai, xj`；如果vi的值不在物理寄存器中，调用`emitLoadFromStack`。（思考： 如果前面不解除volatile寄存器的绑定，这里可能会有什么问题？ 你有更高效的解决方案吗？）
...
这里我们需要关注`RiscvSubroutineEmitter`类。被调用者需要从正确的位置拿到参数，因此需要处理寄存器和临时变量的对应关系，同时作为一个被调用者，我们要从正确的位置返回调用处，因此需要处理和返回地址相关的信息。

1. **处理返回地址：**保存和恢复`ra`寄存器
...
2. **处理函数参数和临时变量的对应关系：**`RiscvSubroutineEmitter`通过成员`nextLocalOffset`和`offsets`管理临时变量在栈上的位置，现在函数的输入参数对应的临时变量也应当纳入`offsets`的管理。比较大的问题来自于栈传递的参数，它们位于当前函数的栈帧之外，难以直接用于存放临时变量（栈帧固定前相对偏移会变化）。所以你需要在真正进入函数主体之前将栈参数的值复制到常规临时变量区域。类似地，对于用寄存器传递的参数，在函数主体前将它们保存在栈上即可。（对于寄存器传参的函数参数，也可以在寄存器分配过程中将对应的物理寄存器和参数临时变量直接绑定），具体地：
