...
### 提示
关于测试样例：
我们的测试脚本会将你的编译器生成的汇编代码与我们提供的运行时框架一起通过 gcc 链接得到可执行文件，检查运行结果。因此，你可以查看测试文件夹中的 `runtime.c`,`runtime.h`,`runtime.s` 来查看我们预定义的函数。

C++框架中：

1. 框架中会在生成的汇编中将函数名前面加上下划线 `_`，如果链接时出现问题请检查是不是下划线导致的。
