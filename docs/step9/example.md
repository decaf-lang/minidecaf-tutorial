...
3. 框架中预留了`NEXT_OFFSET`宏，这个宏会将栈上的参数的相对offset计算出来，并且每次调用会移动到下一个参数的位置，注意将其设置为`Temp`中的offset这一变量。此外还有`is_offset_fixed`这一变量，设置了这个值就能让编译器在后续使用该虚拟寄存器时从栈上offset的位置取出该虚拟寄存器。你不需要真正去计算这个offset，可以直接调用该宏得到。
