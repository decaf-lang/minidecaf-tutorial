...
>
> 另外在 C++ 框架中已经帮助大家实现了用栈传递参数的逻辑，因此大家可以把 **PARAM 指令**理解为 PUSH 指令，按照调用约定从左往右将参数压栈即可（注意用栈传递的逻辑不一定要求参数的值会自动依次按顺序装载到临时变量 _T0, _T1 ... 中）。需要注意的是，在函数调用结束之后需要用 **POP 指令**将这些参数从栈上弹出。代码的实现细节：
>
> （1）src/translation/translation.cpp 中 visit(ast::FuncDefn *f) 确定了参数的在栈上 offset。
>
> （2）src/tac/trans_helper.cpp 中 memoOf(Function *f)) 确定了存储参数值的 Temp 在栈上的位置。
>
> 因此 C++ 框架中大家需要做的事情为，在调用函数时，将参数按照调用约定依次入栈，然后 **CALL LABEL**，调用结束之后将参数从栈上弹出。
...
1. caller-saved 和 callee-saved 寄存器
   ...
   callee 只需要保存 fp 和 ra 寄存器。其中 fp 寄存器中存储调用者栈帧的栈底地址，用于在函数调用结束之后恢复调用者的栈帧，ra 寄存器保存函数返回地址。程序实验框架中提供了数据流分析模块，caller 只需保存活跃变量对应的寄存器内容（数据流分析和活跃变量见 step7）。
